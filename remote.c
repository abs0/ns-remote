...
mozilla_remote_find_window (Display *dpy)
{
  ...
  Window result = 0;
  Window tentative = 0;
  unsigned char *tentative_version = 0;
  ...
  for (i = nkids-1; i >= 0; i--)
	...
	continue;
	  ...
	  strcmp((char *)version, (char *)tentative_version) > 0 )
#else
  ...
  if (result && tentative)
    {
      ...
      fprintf (stderr,
	       ...
	       progname, tentative_version, (unsigned int) tentative,
	       expected_mozilla_version, (unsigned int) result,
	       expected_mozilla_version);
#endif
      XFree (tentative_version);
      return result;
    }
  else if (tentative)
    {
      ...
      fprintf (stderr,
	       ...
	       tentative_version, (unsigned int) tentative);
#endif
      XFree (tentative_version);
      return tentative;
    }
